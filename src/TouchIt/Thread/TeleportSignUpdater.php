                    ...
                    foreach($signs as $sign){
                        ...
                        if($tile instanceof Sign){
                            if(!($target = Server::getInstance()->getLevelByName($sign['target']))){
                                ...
                                $tile->setText("[".$config['name']."]", TouchIt::getLang("update.level.default.line2"), TouchIt::getLang("update.level.limit.line3"), min(count($target->getPlayers()), "[".TouchIt::getConfigProvider()->get("maxPeople", 20))."/".TouchIt::getConfigProvider()->get("maxPeople", 20)."]");
                            }
                        }
                    }
                }
